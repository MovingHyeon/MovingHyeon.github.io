...
하지만 무턱대고 $\mathcal{X}$의 모든 subset위에서 measure를 정의하라고 요구할 수는 없는 노릇이다. Measure가 잘 정의되도록 $\mathcal{X}$의 subset들(전체 혹은 일부)을 모아놓은 집합을 정의할 수 있으며, 이를 바로 $\mathcal{X}$의 $\sigma$-field<sub>$\sigma$-대수</sub>라고 부른다
...
$\sigma$<sub>시그마</sub>는 보통 countable한 집합들의 union<sub>합집합</sub>이 다시 algebra에 있을 때 특별히 붙여주는 이름이다. $\delta$<sub>델타</sub>는 보통 countable한 집합들의 intersection<sub>교집합</sub>이 다시 algebra에 들어있을 때 특별히 붙여주는 이름이다. 이런 쓸데없는 상식을 여럿 알아두면 아무쪼록 도움된다.
...
>
> $\mu$가 probability measure가 아닌 결정적인 이유는 countable additivity가 깨지기 때문이다. $A_n = \\{n\\}$으로 설정하면 $A = \bigcup_{n=1}^\infty A_n = \mathbb{N}$인데, 모든 $n$에 대하여 $\mu(A_n) = 0$이라서 $\sum_{n=1}^\infty \mu(A_n) = 0$이지만 $\mu(A) = 1$이다.
