...
$$ 0\lt a\lt \frac{-2Cov_{\theta_0}(\delta, U)}{Var_{\theta_0}(U)}$$
...
> &= 2X_{(n)}E\left[\frac{\overline{X}}{X_{(n)}}\right] \\
> &= \frac{2}{n}X_{(n)}\sum_i E\left[\frac{X_i}{X_{(n)}}\right] \\
> &= \frac{2}{n}X_{(n)}\sum_i E\left[\frac{X_{(i)}}{X_{(n)}}\right]
> \end{align*}
> $$
>
...
|$\delta\_{a\_0}$|$\displaystyle -\frac{\theta}{(n+1)^4}$|$\displaystyle \frac{n(n+2)\theta^2}{(n+1)^4}$|$\displaystyle \frac{\theta^2}{(n+1)^2}$|
...
>
> 우리는 $g(\theta) = e^{-\theta}$를 예측하고자 한다. 이 수치는 Poisson distribution을 truncate하면서 잃어버린 정보의 비율이다. 생각해보면 잃어버린 정보는 Poisson distribution에서 0일 때이다. Poisson distribution을 각 사람 당 어떤 사고가 발생할 rate $\theta$에 대한 model이라고 생각하면, $g(\theta)$는 무사고율을 나타냄을 알 수 있다. 우리는 이를 예측하기 위한 합당한 estimator를 만들고자 하는 것이다. Model이 $\\{1, 2, \cdots\\}$로 truncate된 Poisson distribution을 사용하는 이유는 0이라는 값이 관측이 불가능한 data가 있기 때문이고 이 상황도 마찬가지이다. 사고의 건수가 0인 사람의 data는 잠재적으로 보고될 수 없기 때문에 data에 없을 것이다. 또 다른 예로는 입원일 수(최소 기간이 1일, 0일 입원은 입원한 것이 아니므로), 공장에서의 각 노동자 사고 수, 각 가구당 전염병 감염 수 등이 있다.
>
> 만약 $\delta(X)$가 unbiased라면 정의에 의해 다음을 만족해야 한다.
>
...
---
다음 post에서는 unbiased estimator의 risk(즉, variance)에 대한 lower bound를 제공하는 매우 유명한 inequality를 알아볼 것이다. 또, 이때 중요한 수치 하나가 소개되는데 관측가능한 data $X$가 알려지지 않는 parameter $\theta$의 information을 얼마나 담고 있는지에 대한 값으로 앞으로의 inference에서 굉장히 많이 활용된다.